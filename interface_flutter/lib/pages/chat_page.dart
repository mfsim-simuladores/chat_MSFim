  ...
  Future<void> _executarAcaoStream(String acao) async {
    ...
    try {
      ...
      await for (String line in resp.stream
        ...
        if (message == null || message.toString().trim().isEmpty) continue;

        setState(() {
          if (event["items"] != null && event["items"] is List){
            _messages.add(
              Message.actionBlock(title: event["title"] ?? "Análise",
              items: List<String>.from(event["items"]),
              footer: event["footer"],
              )
            );
          } else {
            _messages.add(Message.system(message.toString()));
          }
        });
      ...
      setState(() {
        _messages.add(Message.system("Erro de comunicação com o backend."));
      });
    } finally {
      client.close();
    }
